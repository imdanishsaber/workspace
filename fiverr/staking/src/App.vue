...
export default {
    ...
    const providerOptions = {
      walletconnect: {
        ...
        options: {
          ...
          chainId: 137,
          infuraId: "41b807caf34b4af59c20ae9ba1d0e1fe",
  ...
  methods: {
      ...
      if (this.GET_METAMASK_ACCOUNT) {
        ...
        Promise.all([
          ...
          this.GET_STAKING_INSTANCE.methods
            .tokensStakedKaratePandaClubNFT()
            .call(),
    ...
    event_stake() {
      ...
      this.processing = true;
      this.GET_STAKING_INSTANCE.methods
        .stakeBatchKaratePandaClubNFT(ids)
        .send({
          from: this.GET_METAMASK_ACCOUNT,
        })
        .on("transactionHash", (hash) => {
    ...
    event_unstake() {
      ...
      this.processing = true;
      this.GET_STAKING_INSTANCE.methods
        .unstakeBatchKaratePandaClubNFT(ids)
        .send({
          from: this.GET_METAMASK_ACCOUNT,
        })
        .on("transactionHash", (hash) => {
    ...
    onClaim() {
      this.processing = true;
      this.GET_STAKING_INSTANCE.methods
        .harvestBatchKaratePandaClubNFT(this.GET_METAMASK_ACCOUNT)
        .send({
          from: this.GET_METAMASK_ACCOUNT,
        })
        .on("transactionHash", (hash) => {
    ...
    async getNFTUnStaked() {
      ...
      const tokenBalance = await this.GET_NFT_INSTANCE.methods
        .tokensOfOwner(this.GET_METAMASK_ACCOUNT)
        .call();
      tokenBalance.forEach(async (nftNo) => {
        ...
        let isImage = this.isImageUrl(image.image);
    ...
    async getNFTStaked() {
      ...
      const nftList = await this.GET_STAKING_INSTANCE.methods
        .stakedNFTSByUserKaratePandaClubNFT(this.GET_METAMASK_ACCOUNT)
        .call();
    ...
    async getImageHash(metadata) {
      ...
      try {
        ...
        let responseURL = responseJson.image || responseJson.animation_url;
        if (responseURL) {
          let [ips, imageURI] = responseURL.split("//");
          let ipfs = ips.replace(":", "/");
          let url = imageURI;
          return {
      ...
      stakedList.forEach(async (nftNo) => {
        if (nftNo != "0") {
          let reward = await this.GET_STAKING_INSTANCE.methods
            .pendingRewardsKaratePandaClubNFT(this.GET_METAMASK_ACCOUNT, nftNo)
            .call();
