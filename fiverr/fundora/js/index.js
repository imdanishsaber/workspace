...
const POOLABI = [
  ...
  },
  {
    ...
    outputs: [
      ...
      { internalType: "uint256", name: "claimLockTimestamp", type: "uint256" },
    ],
    stateMutability: "view",
    type: "function",
  },
  ...
  methods: {
    async readValues() {
      Promise.all([
        ...
        ([
          ...
          this.P1totalInvested = this.fixedDecimal(
            parseFloat(P1totalInvested / 1e18)
          );

          this.P1totalRewardsPaid = this.fixedDecimal(
            parseFloat(P1totalRewardsPaid / 1e18)
          );

          this.P1Stake = this.fixedDecimal(parseFloat(P1Stake.stake / 1e18));

          this.P1Reward = this.fixedDecimal(
            parseFloat(P1Stake.unclaimedReward / 1e18)
          );
          let futureTimestamp = Number(P1Stake.timestamp) + 3888000;
          if (futureTimestamp > Math.floor(new Date().getTime() / 1000))
            this.earyWithdraw =
              this.convertTimestampToDaysHours(futureTimestamp);
          else this.earyWithdraw = "";
          this.P2Allowance = Number(
            this.fixedDecimal(parseFloat(P2Allowance / 1e18))
          );
          this.P2totalInvested = this.fixedDecimal(
            parseFloat(P2totalInvested / 1e18)
          );
          this.P2totalRewardsPaid = this.fixedDecimal(
            parseFloat(P2totalRewardsPaid / 1e18)
          );
          this.P2Stake = this.fixedDecimal(parseFloat(P2Stake.stake / 1e18));
          this.P2Reward = this.fixedDecimal(
            parseFloat(P2Stake.unclaimedReward / 1e18)
          );
        }
      );
    },
    ...
    onAction() {
      ...
      if (this.pool == 1) {
        ...
        address = P1Address;
        if (this.isWithdraw) {
          this.onWithdraw(instance, address);
        } else {
          if (this.P1Allowance >= this.buyAmount)
            this.onDeposit(instance, address);
        ...
        if (this.isWithdraw) {
          ...
          if (this.P2Allowance >= this.buyAmount)
            this.onDeposit(instance, address);
          else this.onApprove(instance, address);
        }
      }
