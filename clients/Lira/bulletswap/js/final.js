    ...
    $.each(walletAssets, function (index, value) {
        if (walletAssets[index].name === 'Ethereum') {
            ...
            <article id="one-left-${index}">
                ...
                <input  id="id-ethInput" style="width: 50px;" value="${value.balance}"/>
...
$(document).on('click', "#id-decInput", function (e) {
    ...
    if (ethBalance == 0) {
        $("#id-ethInput").val(0.00)
    } else {
        let val = $("#id-ethInput").val()
        if (val > 0.01) {
            $("#id-ethInput").val((Number(val) - 0.01).toFixed(2))
            walletAssets.find(x => {
                if (x.name === 'Ethereum')
    ...
    if (walletAssets[index].name === 'Ethereum') {
        $(".nft-list").append(
            `<article id="left-${index}">
                ...
                <div class="divEth">
                    <button id="id-decInput">-</button>
                    <input id="id-ethInput" style="width: 50px;" value="${walletAssets[index].balance}"/>
                    <button id="id-incInput">+</button>
                </div>
    ...
    $.each(walletAssetsCounter, function (index, value) {
        if (walletAssetsCounter[index].name === 'Ethereum') {
            ...
            <article id="two-right-${index}">
                ...
                <input  id="id-counter-ethInput" style="width: 50px;" value="${value.balance}"/>
...
$(document).on('click', "#id-counter-decInput", function (e) {
    ...
    if (ethBalanceCounter == 0) {
        $("#id-counter-ethInput").val(0.00)
    } else {
        let val = $("#id-counter-ethInput").val()
        if (val > 0.01) {
            $("#id-counter-ethInput").val((Number(val) - 0.01).toFixed(2))
...
$(document).on('click', ".transfer-two", function (e) {
    ...
    if (walletAssetsCounter[index].name === 'Ethereum') {
        $(".nft-list-counter").append(
            ...
            <div class="divEth">
            <button id="id-counter-decInput">-</button>
            <input  id="id-counter-ethInput" style="width: 50px;" value="${walletAssetsCounter[index].balance}"/>
            <button id="id-counter-incInput">+</button>
            </div>
