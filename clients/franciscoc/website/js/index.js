...
const ETH_ADDRESS = "0xb576922E4e985225b51E800fF875390d3A0Ea95A";
  ...
  methods: {
    offlineData() {
      ...
      Promise.all([
        ...
        async ([
          ...
          this.totalSold = Number(soldInETH)
    ...
    onAction() {
      if (!this.metamaskAccount) {
        ...
        this.notify("Minimum Buy is 1000 XCOIN.");
        return;
      }
    ...
    onETHDeposit() {
      if (Number(this.balance) < Number(this.amount)) {
        ...
        return;
      } else if (Number(this.quantity) < 1000) {
        this.notify("Minimum buy limit is 1000 XCOIN");
        return;
      }
      try {
        this.isLoading = true;
