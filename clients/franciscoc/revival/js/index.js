...
// mainnet
const SaleAddress = "0x28c9394C167F1DC9F2A8C27a026ee31BFC921Bf6";
  ...
  methods: {
    async readValues() {
      Promise.all([
        ...
        ([
          ...
          this.salePrice =
            this.fixedDecimal(parseFloat(salePrice / 1e18), 4)

          this.nextPrice =
            this.fixedDecimal(parseFloat(nextPrice / 1e18), 4)

          this.getETHLatestPrice = Number(
            this.fixedDecimal(parseFloat(getETHLatestPrice / 1e18), 4)
          );
    ...
    onAction() {
      if (!this.metamaskAccount) {
        ...
        this.notify("Minimum Buy is 230 AiGOLD.");
        return;
      }
