...
var app = new Vue({
      ...
      Promise.all([
        ...
        ([
          ...
          if (ETHbalance == 0) {
            ...
            this.ETHbalance = this.fixedDecimal(parseFloat(ETHbalance / 1e18), 2);
          }

          if (USDTbalance == 0) {
            this.USDTbalance = USDTbalance;
          } else {
            this.USDTbalance = this.fixedDecimal(parseFloat(USDTbalance / 1e18), 2);
          }

          this.USDTAllowance = Number(
            this.fixedDecimal(parseFloat(USDTAllowance / 1e18))
          );
          this.ethRate = Number(
            this.fixedDecimal(parseFloat(ethRate / 1e18))
          );
        }
      );
    },
  ...
  computed: {
    quantity() {
      if (this.type === 1)
        return (Number(this.amount) / Number(this.priceInETH)).toFixed(2)
      else
        return (Number(this.amount) / Number(this.priceInETH * this.ethRate)).toFixed(2)
    }
  },
});
