...
var app = new Vue({
  ...
  methods: {
    offlineData() {
      ...
      Promise.all([
        ...
        async ([
          ...
          this.totalSold = Number(this.totalUsdValueForPresale) - Number(ETH_SOLD_USD)
    ...
    async readValues() {
      ...
      Promise.all([
        ...
        ([
          ...
          this.salePrice =
            this.fixedDecimal(parseFloat(salePrice / 1e18), 10)

          this.nextPrice =
            this.fixedDecimal(parseFloat(nextPrice / 1e18), 10)

          this.getLatestPrice = Number(
            this.fixedDecimal(parseFloat(getLatestPrice / 1e18), 10)
          );
    ...
    onAction() {
      if (!this.metamaskAccount) {
        ...
        this.notify("Minimum Buy is 2500 Viking.");
        return;
      }
    ...
    onETHDeposit() {
      if (Number(this.balance) < Number(this.amount)) {
        ...
        this.notify("Minimum buy limit is 2500 Viking");
        return;
      }
      try {
        this.isLoading = true;
    ...
    onApprove() {
      ...
      try {
        this.isLoading = true;
        this.TOKEN_OBJ.methods
          .approve(ETH_ADDRESS, "1000000000000000000000000000")
          .send({
            from: this.metamaskAccount,
          })
          .on("transactionHash", (hash) => {
  ...
  computed: {
    ...
    bonus() {
      if (this.quantity)
        return this.fixedDecimal((this.quantity * 0.1), 0)
      else return 0
    },
    progress() {
